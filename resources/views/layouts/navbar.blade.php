...
<nav class="bg-white shadow" x-data="{openMenu: false}">
      ...
      <div class="flex h-16 justify-between">
        <div class="flex">
          ...
          <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
            ...
            @if(Auth::check())
                <a href="{{ route('dashboard') }}" class="inline-flex items-center border-b-2 border-green-500 px-1 pt-1 text-sm font-medium text-gray-900">Dashboard</a>
            @else
                <a href="{{ route('home') }}" class="inline-flex items-center border-b-2 {{ $routeName == 'home' ? 'border-green-500' : 'border-transparent' }} px-1 pt-1 text-sm font-medium text-gray-500 hover:border-gray-300 hover:text-gray-700" >Accueil</a>
            @endif
        ...
        @if(Auth::check())
            ...
            <div class="relative ml-3" x-data="{open: false}" @mousedown.outside='open= false'>
                <div>
                <button x-on:click='open = !open' type="button" class="flex rounded-full bg-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                    <span class="sr-only">Open user menu</span>
                    <img class="h-8 w-8 rounded-full" src="data:image/png;base64,{{ Auth::user()->photo }}" alt="">
                </button>
                </div>
                ...
                <a href="{{ route('dashboard.account') }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-1">Mon compte</a>
                <a href="{{ route('logout') }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-2">Déconnexion</a>
                </div>
            </div>
            </div>
        @endif
        ...
        <div class="-mr-2 flex items-center sm:hidden" >
          <!-- Mobile menu button -->
          <button x-on:click="openMenu= !openMenu" type="button" class="inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500" aria-controls="mobile-menu" aria-expanded="false">
    ...
    <div x-cloak x-show='openMenu' class="sm:hidden" id="mobile-menu">
      <div class="space-y-1 pb-3 pt-2">
        ...
        <a href="#" class="block border-l-4 border-green-500 bg-green-50 py-2 pl-3 pr-4 text-base font-medium text-green-700">Dashboard</a>
      ...
      <div class="border-t border-gray-200 pb-3 pt-4">
        ...
        <div class="mt-3 space-y-1">
          <a href="{{route('dashboard')}}" class="block px-4 py-2 text-base font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-800">Dashboard</a>
          <a href="{{ route('dashboard.account') }}" class="block px-4 py-2 text-base font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-800">Mon compte</a>
          <a href="{{ route('logout') }}" class="block px-4 py-2 text-base font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-800">Déconnexion</a>
        </div>
      </div>
    </div>
  </nav>
